...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Auto-fill buyer and seller
    const requestorName = document.getElementById('requestor_name');
    const buyer = document.getElementById('buyer');
    const seller = document.getElementById('seller');
    if (requestorName && buyer && seller) {
        requestorName.addEventListener('input', function() {
            buyer.value = this.value;
            seller.value = this.value;
            validateField(buyer);
            validateField(seller);
        });
    }
    ...
    if (valuationAmount) {
        valuationAmount.addEventListener('input', function() {
            ...
            if (value > 0) {
                ...
                amountInWords.textContent = '(Rials Omani ' + numberToWords(value) + ')';
            } else {
    ...
    function numberToWords(number) {
        ...
        if (whole > 0) {
            const chunks = String(whole).padStart(Math.ceil(String(whole).length / 3) * 3, '0').match(/.{1,3}/g).reverse();
            chunks.forEach((chunk, i) => {
                ...
                if (chunkWords.length) {
                    words.push(chunkWords.join(' ') + (thousands[i] ? ' ' + thousands[i] : ''));
                }
            });
